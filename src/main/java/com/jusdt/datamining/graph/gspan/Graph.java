	...
	public boolean hasEdge(int x, int a, int y) {
		...
		for (int i = 0; i < nodeLabels.size(); i++) {
			...
			for (int j = 0; j < edgeNexts.get(i).size(); j++) {
				// 从此端点的所连接的点去比较对应的点和边
				if (edgeLabels.get(i).get(j) == a && nodeLabels.get(edgeNexts.get(i).get(j)) == t) {
					isContained = true;
					return isContained;
				}
			}
	...
	public Graph constructGraph(GraphData gd) {
		...
		for (int i = 0; i < gd.getEdgeLabels().size(); i++) {
			if (gd.getEdgeVisibles().get(i)) {
				...
				graph.edgeNexts.get(gd.getEdgeX().get(i)).add(gd.getEdgeY().get(i));
				graph.edgeNexts.get(gd.getEdgeY().get(i)).add(gd.getEdgeX().get(i));
			}
		}
