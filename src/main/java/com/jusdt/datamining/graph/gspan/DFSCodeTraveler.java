	...
	private void dfsSearch(Stack<Integer> stack, int currentPosition, int next) {
		...
		while (!stack.isEmpty()) {
			...
			for (int i = 0; i < graph.edgeNexts.get(x).size(); i++) {
				...
				if (g2s[y] < 0) {
					// 新建这条边五元组
					Edge e = new Edge(g2s[x], next, graph.nodeLabels.get(x), graph.edgeLabels.get(x).get(i),
							graph.nodeLabels.get(y));

					// 与相应位置的边做比较，如果不是最小则失败
					int compareResult = e.compareWith(edgeSeqs.get(currentPosition));
					...
					// 新建这条边五元组
					Edge e = new Edge(g2s[x], g2s[y], graph.nodeLabels.get(x), graph.edgeLabels.get(x).get(i),
							graph.nodeLabels.get(y));
